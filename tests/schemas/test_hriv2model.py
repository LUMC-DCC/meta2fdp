...
model_config_path = pathlib.Path("tests/data/config/model_config_test.yaml")
...
class test_Hriv2Model(unittest.TestCase):
    ...
    def test_lang_literals_(self):
        ...
        self.assertEqual(
            expected_result, self.model.lang_literals(test_metadata, "tag")
        )
        ...
        self.assertEqual(
            expected_result_override,
            self.model.lang_literals(test_metadata_override, "tag"),
        )

        test_metadata_nolang = pd.Series(data=["no_label"], index=["tag"])
        expected_result_nolabel = [LiteralField(value="no_label")]
        self.assertEqual(
            expected_result_nolabel,
            self.model.lang_literals(test_metadata_nolang, "tag"),
        )
...
<http://fdp.org/new> a v:Kind ;
    ...
    v:hasEmail <mailto:anemail@test.io> .""",
        ...
        result_graph = self.model.convert_class_to_rdf(
            vcard, URIRef("http://fdp.org/new")
        )
        unique_triples = expected_graph ^ result_graph
        self.assertIs(
            len(unique_triples),
            0,
            msg=f"Serialized graphs are not the same! These are the unexpected triples{unique_triples.serialize()}",
        )
    ...
    def test_instantiate_HRIDataset(self):
        ...
        result = self.model.instantiate_HRIDataset(
            dataset_metadata, vcard, agent, creators
        )
        ...
        self.assertEqual(
            result.keyword,
            [LiteralField(value="keyword1"), LiteralField(value="keyword2")],
        )
