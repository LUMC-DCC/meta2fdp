...
class Hriv2Model(AbstractModel):
        ...
        if colname in metadata.index:
            ...
            for langtag in self.model_config["langtags"].split(","):
                ...
                if property_colname in metadata.index:
                    langtagged_literal = LiteralField(value=metadata.loc[property_colname], language=langtag)
    ...
    def instantiate_HRIVcard(self, metadata: pd.Series, prefix="contactPoint") -> HRIVCard:
        ...
        formatted_name=self.lang_literals(metadata, prefix +"_name").pop() # formatted_name accepts a single property
        )
        return vcard
    ...
    def instantiate_HRICatalog(self, metadata: pd.Series, contact_point: HRIVCard, publisher: HRIAgent) -> HRICatalog:
    ...
    def instantiate_HRIDataset(self, metadata: pd.Series, contact_point: HRIVCard, publisher: HRIAgent, creators: list[HRIAgent]):
        dataset = HRIDataset(
        title=self.lang_literals(metadata, "title"),
        description=self.lang_literals(metadata, "description"),
        ...
        number_of_records=LiteralField(value=str(metadata.loc["numberOfRecords"]), datatype=XSD.nonNegativeInteger), # relevant in current samplenavigator usecase
        number_of_unique_individuals=LiteralField(value=str(metadata.loc["numberOfUniqueIndividuals"]), datatype=XSD.nonNegativeInteger), # relevant in current samplenavigator usecase
