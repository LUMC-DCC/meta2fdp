...
class FDPClient(AbstractClient):
    ...
    def get_api_token(self):
        ...
        :rtype: String
        """
        self.token = self.config.keyringprovider.get(
            self.config.get_fdp_url(),
            self.config.environmentprovider.get("FDP_USERNAME"),
        )

        # Check if api-key works:
        url = self.config.get_fdp_url() + "/users/current"
        data = {}
        ...
        response = requests.request(
            ...
            headers={
                ...
                + self.config.keyringprovider.get(
                    self.config.get_fdp_url() + "/users/current",
                    self.config.environmentprovider.get("FDP_USERNAME"),
                ),
            },
        )
    ...
    def post_resource(self, data, resource_type):
        ...
        try:
            ...
            raise FileNotFoundError(
                "Error getting location url, if this is because of badly formed metadata this is the SHACL validation report:\n {}".format(
                    response.text
                )
            )
    ...
    @override
    def upload_resource(
        self, graph, purl, resource_type="Catalog", resource_name="Catalog"
    ):
        """
    ...
    """
        ...
        if len(qres.bindings) == 1:
            ...
            LookupError(
                "multiple identifiers found, are you sure this is a unique resource?"
            )
    ...
    def update_resource(self, url, dcat_type, new_graph: Graph):
        ...
        if (
            new_id != FDP_id
        ):  # HACK IF THE RESOURCE HAS NO DCTERMS:IDENTIFIER IT WILL AUTOMATICALLY MATCH
            raise IndexError(
                "resource id {} does not match with current update!".format(url)
            )
        else:
            # assume that object is fdp/new URL
            utils.subject_replace(
                url,
                node_id=self.URL + "/" + "new",
                dcat_type=dcat_type,
                graph=new_graph,
            )
            ...
            old_Kinds = [
                triple
                for triple in FDP_graph.triples(
                    (None, RDF.type, URIRef("http://www.w3.org/2006/vcard/ns#Kind"))
                )
            ]
    ...
    def get_dataset_id_purls(
        self, graph: Graph
    ) -> dict:  # FIXME update based on content in replace_datasets in csvparser
        """
